...
class MainTest {
    ...
    fun `parsing input`() {
        ...
        assertEquals(expectedResult, sampleData.parse() )
    }
    @Test
    fun `binary to integer`() {
        val result = "000000000000000000000000000001001001".binaryToLong()
    ...
    fun `processing sample data program`() {
        ...
        sampleData.parse().forEach{program ->  program.process(memory)}
        assertEquals(165, memory.values.sum())
    }
    @Test
    fun `part one`() {
        val memory = mutableMapOf<Long, Long>()
        day14Data.parse().forEach{program ->  program.process(memory)}
    ...
    fun `a mask containing one X is converted to a list of maps containing two masks`() {
        val result:List<String> = convertMask("00000X11111")
    ...
    fun `a mask containing two X is converted to a list of maps containing four masks`() {
        val result:List<String> = convertMask("00000X11X11")
    ...
    fun `processing sample data program for part two`() {
        ...
        """.trimIndent().split("\n")
        val memory = mutableMapOf<Long, Long>()
        sampleData.parse().forEach{program ->  program.process2(memory)}
    ...
    fun `part two`() {
        val memory = mutableMapOf<Long, Long>()
        day14Data.parse().forEach{program ->  program.process2(memory)}
